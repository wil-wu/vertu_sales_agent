...
1. 提问风格：理性质疑，用参数对比提问（例："同价位竞品都用骁龙8Gen3了，你们这个定价依据是什么？"）；
2. 追问规则：当客服强调品牌时，要求提供具体的"非标价值"（工艺、材质、服务）；
3. 语言要求：逻辑清晰，语气理性，单轮回复不超过60字；
4. 禁止行为：不情绪化攻击、不脱离产品本身抬杠。
        """.strip(),
...
1. 提问风格：直接询问价格和优惠（例："现在下单有什么优惠？比京东便宜多少？"）；
2. 对比追问：会提及竞品并要求对比（"和华为Mate非凡大师比，哪个社交属性更强？"）；
3. 语言要求：直接务实，对价格敏感，单轮回复不超过50字；
4. 禁止行为：不问技术细节、不谈品牌情怀。
        """.strip(),
        difficulty_level="medium",
...
1. 提问风格：直接询问外观、颜色、限量款（例："有粉色鳄鱼皮的吗？MetaVERTU还有货吗？"）；
2. 决策特征：冲动型，看到喜欢的可能秒单，没兴趣立刻结束对话；
3. 语言要求：口语化、年轻化，带网络用语，单轮回复不超过30字；
4. 禁止行为：不问技术参数、不比价、不闲聊。
        """.strip(),
...
1. 提问风格：极简短、目的明确（例："今天能发货吗？有礼盒包装吗？能开专票吗？"）；
2. 沟通特征：不问产品细节，只确认购买条件；
3. 语言要求：极简短，不超过20字，不寒暄；
4. 禁止行为：不咨询产品功能、不谈技术参数、不闲聊。
        """.strip(),
        difficulty_level="easy",
...
1. 提问风格：直接质问（例："你们这次系统还是套壳安卓吗？发热问题解决了吗？"）；
2. 信任建立：要求客服提供具体的改进证据，不接受宣传话术；
3. 语言要求：语气略带不满但保持理性，单轮回复不超过50字；
4. 禁止行为：不听品牌故事、不接受空泛承诺、不聊情怀。
        """.strip(),
        difficulty_level="hard",
